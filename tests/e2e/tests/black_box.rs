...
async fn test_real_ebpf_20k_events(ctx: Context) {
    ...
    ctx.apply(&receiver_svc).await.expect("Deploy receiver service");
    ctx.wait_ready("deployment/test-receiver").await.expect("Wait for receiver");
    ...
    ctx.apply(&polku_svc).await.expect("Deploy POLKU service");
    ctx.wait_ready("deployment/polku").await.expect("Wait for POLKU");

    // Port forward to both POLKU (to send) and receiver (to verify)
    let polku_pf = ctx.port_forward("svc/polku", 50051).await.expect("Port forward POLKU");
    let receiver_pf = ctx.port_forward("svc/test-receiver", 9001).await.expect("Port forward receiver");
    ...
    // Connect to POLKU
    let mut client = PolkuClient::connect(&polku_addr).await.expect("Connect to POLKU");
    ...
    let initial_health = receiver_client.health(HealthRequest {}).await.expect("Initial health");
    ...
    for batch_num in 0..200 {
        ...
        if batch_num % 20 == 0 {
            eprintln!(">>> Sent batch {} ({} events so far)", batch_num, total_sent);
        }
    }

    let send_time = start.elapsed();
    let send_rate = total_sent as f64 / send_time.as_secs_f64();
    eprintln!(">>> Sent {} events in {:?} ({:.0} events/sec)", total_sent, send_time, send_rate);

    // Wait for events to propagate through POLKU to receiver
    tokio::time::sleep(Duration::from_secs(3)).await;

    // Verify events arrived at receiver via health endpoint
    let final_health = receiver_client.health(HealthRequest {}).await.expect("Final health");
    let final_count = final_health.into_inner().events_processed;

    let received = final_count - initial_count;
    eprintln!(">>> Receiver processed {} events (initial={}, final={})", received, initial_count, final_count);

    // Verify we received the events
    assert_eq!(total_sent, total, "Should have sent {} events", total);
    assert!(received >= (total_sent as u64 * 95 / 100),
        "Should have received at least 95% of events: sent={}, received={}", total_sent, received);

    eprintln!(">>> SUCCESS: {} real eBPF events flowed through POLKU to receiver", received);
}
...
async fn test_kernel_syscall_events(ctx: Context) {
    ...
    assert_eq!(response.event_ids.len(), 5, "All 5 syscall events should be accepted");
    assert!(response.errors.is_empty(), "No errors expected");

    tracing::info!("SUCCESS: {} kernel syscall events sent", response.event_ids.len());
}
...
async fn test_network_events(ctx: Context) {
    ...
    assert_eq!(response.event_ids.len(), 100, "All 100 network events should be accepted");
    assert!(response.errors.is_empty(), "No errors expected");
...
async fn test_streaming_500k_events(ctx: Context) {
    ...
    eprintln!(">>> [STREAMING] Initial receiver event count: {}", initial_count);
    ...
    eprintln!(">>> [STREAMING] Sending via bidirectional streaming (batch_size={})...", batch_size);
    let start = std::time::Instant::now();

    let response = client.stream_events(events, batch_size).await.expect("Stream should succeed");
    let total_sent = response.event_ids.len();

    let send_time = start.elapsed();
    let send_rate = total_sent as f64 / send_time.as_secs_f64();
    eprintln!(">>> [STREAMING] Sent {} events in {:?} ({:.0} events/sec)", total_sent, send_time, send_rate);
    ...
    let received = final_count - initial_count;
    eprintln!(">>> [STREAMING] Receiver processed {} events (initial={}, final={})", received, initial_count, final_count);
    ...
    eprintln!(">>> [STREAMING] SUCCESS: {} events at {:.0} events/sec", received, send_rate);
}
